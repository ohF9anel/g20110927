...
class EyeDataGrid
{
	...
	*/
	public function __construct(EyeMySQLAdap $_db, $image_path = '')
	{
		...
		if (empty($image_path))
			$this->image_path = '../classes/eyesis/images/';    //locatie van de afbeeldingen bekeken vanuit de admintemplates map
		else
	...
	*/
	...
	{
            $headerString =""; //deze is de vervanging van echo ==> dus '$headerString.= ' vervangen door echo
		...
		{
			$this->column_count++;
			$headerString.=  '<td class="tbl-header">&nbsp;</td>';
		}
		...
		{
			$this->column_count++;
			$headerString.=  '<td class="tbl-header tbl-checkall"><input type="checkbox" name="checkall" onclick="tblToggleCheckAll()"></td>';
		}
		...
		foreach ($headers as $t)
		{
			...
			if ($this->hide_order)
				$headerString.=  '<td class="tbl-header">' . $header; // Prevent the user from changing order
			else {
				...
				$headerString.=  '<td class="tbl-header"><a href="javascript:;" onclick="tblSetOrder(\'' . $t . '\', \'' . $order . '\')">' . $header . "</a>";

				// Show the user the order image if set
				if ($this->order and $this->order['Column'] == $t)
					$headerString.=  '&nbsp;<img src="' . $this->image_path . 'sort_' . strtolower($this->order['Order']) . '.gif" class="tbl-order">';
			}
	...
	*/
	...
	{
            $footer = ""; //deze is de vervanging van echo ==> dus '$foorter.= ' vervangen door echo
		...
		$footer.=  '<tfoot><tr class="tbl-footer"><td class="tbl-nav" colspan="' . $this->column_count . '"><table width="100%" class="tbl-footer"><tr><td width="33%" class="tbl-found">Found <em>' . $this->row_count . '</em> results';

		if ($this->row_count > 0)
			$footer.= ', showing <em>' . $first . '</em> to <em>' . $last . '</em>';

		$footer.= '</td><td wdith="33%" class="tbl-pages">';

		// Handle results that span multiple pages
		if ($this->row_count > $this->results_per_page)
		{
			if ($this->page > 1)
				$footer.= '<a href="javascript:;" onclick="tblSetPage(1)"><img src="' . $this->image_path . 'arrow_first.gif" class="tbl-arrows" alt="&lt;&lt;" title="First Page"></a><a href="javascript:;" onclick="tblSetPage(' . ($this->page - 1) . ')"><img src="' . $this->image_path . 'arrow_left.gif" class="tbl-arrows" alt="&lt;" title="Previous Page"></a>';
			else
				$footer.= '<img src="' . $this->image_path . 'arrow_first_disabled.gif" class="tbl-arrows" alt="&lt;&lt;" title="First Page"><img src="' . $this->image_path . 'arrow_left_disabled.gif" class="tbl-arrows" alt="&lt;" title="Previous Page">';
      ...
      for ($i = $startpage; $i <= $endpage; $i++)
			{
				if ($i == $this->page)
					$footer.= '&nbsp;<span class="page-selected">' . $i . '</span>&nbsp;';
				else
					$footer.= '&nbsp;<a href="javascript:;" onclick="tblSetPage(' . $i . ')">' . $i . '</a>&nbsp;';
			}

			if ($this->page < $pages)
				$footer.= '<a href="javascript:;" onclick="tblSetPage(' . ($this->page + 1) . ')"><img src="' . $this->image_path . 'arrow_right.gif" class="tbl-arrows" alt="&gt;" title="Next Page"></a><a href="javascript:;" onclick="tblSetPage(' . $pages . ')"><img src="' . $this->image_path . 'arrow_last.gif" class="tbl-arrows" alt="&gt;&gt;" title="Last Page"></a>';
			else
				$footer.= '<img src="' . $this->image_path . 'arrow_right_disabled.gif" class="tbl-arrows" alt="&gt;" title="Next Page"><img src="' . $this->image_path . 'arrow_last_disabled.gif" class="tbl-arrows" alt="&gt;&gt;" title="Last Page">';
		}

		$footer.= '</td><td width="33%" class="tbl-page">';
		...
		if ($pages > 0)
		{
			...
			if (!$this->hide_page_list and $pages > 1)
			{
				...
				$footer.= '<select name="tbl-page" onchange="tblSetPage(this.options[this.selectedIndex].value)">';
				for ($x = 1; $x <= $pages; $x++)
				{
					$footer.= '<option value="' . $x . '"';
					if ($x == $this->page)
				...
				$footer.= $this->page; // Just write the page number, nothing to fancy

			$footer.= ' of ' . $pages;
		}

		$footer.= '</td></tr></table></td></tr></tfoot>';
	...
	*/
	...
	{
            $controls =""; //deze is de vervanging van echo ==> dus '$controls.= ' vervangen door echo
			// Add controls as needed
			if (count($this->controls) > 0)
			{
				$controls.= '<td class="tbl-controls">';
				foreach ($this->controls as $ctl)
					$controls.= $this->parseVariables($row, $ctl);
				$controls.= '</td>';
			}
            return $controls;
	}
	...
	*
        * @return string
        * @param string $queryOpgave      De uit te voeren query
        */
	public function printTable($queryOpgave = "")
	{
            $table ="";//deze is de vervanging van echo ==> dus '$table.= ' vervangen door echo
		// Set the limit
		...
		if (!$this->result)
		{
			$table .= '<div style="color: red; font-weight: bold; border: 2px solid red; padding: 10px;">Oops! We ran into a problem while trying to output the table. <a href="javascript:;" onclick="tblReset()">Click here</a> to reset the table or <a href="javascript:;" onclick="alert(\'' . preg_replace('[\'"]', '', $this->_db->error()) . '\')">here</a> to review the error.</div>';
			return;
		}

		// Count the number of rows without the limit clause
		$this->row_count = (int) $this->_db->selectOneValue('COUNT(*)', $this->select_table, $filter_query); 
		// Old code which does not support large data sets: $this->_db->countRows($query);

		if (!$this->isAjaxUsed())
		{
			// Print out required javascript functions
			$table .= $this->printJavascript(); 
			//in de functie printJavascript moet de $javascript vervangen worden door echo. Op deze lijn '$table .= ' verwijderen
			$table .= '<script type="text/javascript">function updateTable() { window.location = "?" + params; }</script>';
		}

		$table .= '<form action="#" name="dg" id="dg">';

		// Output the create button
		if ($this->create_button)
			$table .= '<span class="tbl-create"><a ' . $this->create_button['Action'] . ' title="' . $this->create_button['Text'] . '"><img src="' . $this->image_path . $this->img_create . '" class="tbl-create-image">' . $this->create_button['Text'] . '</a></span>';

		// Output the reset button
		if ($this->reset_button)
			$table .= '<span class="tbl-reset"><a href="javascript:;" onclick="tblReset()" title="' . $this->reset_button .'"><img src="' . $this->image_path . $this->img_reset . '" class="tbl-reset-image">' . $this->reset_button .'</a></span>';
		...
		if ($this->row_count == 0)
			$table .= '<tr><td colspan="' . $this->column_count . '" class="tbl-noresults">' . self::TXT_NORESULTS . '</td></tr>';
		else {
			$i = 0; $first = 0; $last = 0;

			while ($row = $this->_db->fetchAssoc($this->result))
			{
				$table .= '<tr class="tbl-row tbl-row-' . (($i % 2) ? 'odd' : 'even'); // Switch up the bgcolors on each row

				// Handle row selects
				if ($this->row_select)
					$table .= ' tbl-row-highlight" onclick="' . $this->parseVariables($row, $this->row_select);
				...
				if ($this->show_row_number)
					$table .= '<td class="tbl-row-num">' . $line . '</td>';

				if ($this->show_checkboxes)
					$table .= '<td align="center"><input type="checkbox" class="tbl-checkbox" id="checkbox" name="tbl-checkbox" value="' . $row[$this->primary] . '"></td>';

				foreach ($row as $key => $value)
				{
					// gb 22:05 zondag 11 september 2011
						...
						switch ($type)
						{
							...
							default:
								...
								break;
                                        ...
                                        if(strlen($value) >80 && strpos($value, '<a href=')===false){
                                            $table .= '<td class="tbl-cell" title="'.$value.'">' . substr($value,0,80) . ' ...</td>';
                                        }else{
                                            $table .= '<td class="tbl-cell">' . $value . '</td>';
                                        }
				}

				$table .=$this->buildControls($row);
		...
		$table.= $this->buildFooter($i, $first, $last);
	...
	public function printJavascript()
	{
		if ($this)
		{
			...
			$filter = (($this->filter) ? implode(':', $this->filter) : '');
                ...
                $javascript =""; //deze is de vervanging van echo ==> dus '$javascript.= ' vervangen door echo

		$javascript .= "<script type=\"text/javascript\">\n";
		$javascript .= "var params = ''; var tblpage = '" . $page . "'; var tblorder = '" . $order . "'; var tblfilter = '" . $filter . "';\n";
		$javascript .= "function tblSetPage(page) { tblpage = page; params = '&page=' + page + '&order=' + tblorder + '&filter=' + tblfilter; updateTable(); }\n";
		$javascript .= "function tblSetOrder(column, order) { tblorder = column + ':' + order; params = '&page=' + tblpage + '&order=' + tblorder + '&filter=' + tblfilter; updateTable(); }\n";
		$javascript .= "function tblSetFilter(column) { val = document.getElementById('filter-value-' + column).value; tblfilter = column + ':' + val; tblpage = 1; params = '&page=1&order=' + tblorder + '&filter=' + tblfilter; updateTable(); }\n";
		$javascript .= "function tblClearFilter() { tblfilter = ''; params = '&page=1&order=' + tblorder + '&filter='; updateTable(); }\n";
		...
		$javascript .= "function tblReset() { params = '&page=1'; updateTable(); }\n";
		$javascript .= "</script>\n";
